...
def load_config(path: str | Path) -> PlatformConfig:
    ...
    if suffix in {".yaml", ".yml"}:
        ...
        msg = f"Unsupported config extension: {suffix}"
    ...
    return PlatformConfig.model_validate(cast(dict[str, Any], raw_data))


def save_config_snapshot(config: PlatformConfig, path: str | Path) -> None:
    """Save a validated config as YAML for reproducibility."""
